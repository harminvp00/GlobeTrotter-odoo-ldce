...
export const RegisterForm: React.FC<RegisterFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ general: err.message || 'Registration failed. Email might already be in use.' });
    } finally {
  ...
  return (
    <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-8 border border-purple-50">
      <div className="text-center mb-8">
        ...
        <h2 className="text-3xl font-extrabold text-purple-950">Join GlobeTrotter</h2>
        <p className="text-purple-600/80 mt-2">Start mapping your world itineraries today</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          <label htmlFor="name" className="block text-sm font-semibold text-purple-950 mb-1">
            Full Name
          </label>
          <input
            id="name"
            ...
            className={`w-full px-4 py-2.5 rounded-lg border focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition-colors ${
              errors.name ? 'border-red-300 bg-red-50/30' : 'border-purple-100'
            }`}
        ...
        <div>
          <label htmlFor="password" className="block text-sm font-semibold text-purple-950 mb-1">
            Password
          </label>
          <input
            id="password"
